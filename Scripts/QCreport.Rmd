...
# when quant dir is fastqname:
# mapping_file <- sample_names %>%
#   sapply(function(id) grep(id, mapping_file[[1]])) %>%
#   unlist %>% mapping_file[.,]

# mapping_file <- sapply(sample_names, 
#                        function(id) mapping_file[[1]] %in% id) %>% apply(1,which) %>% mapping_file[.,]

mapping_file <- sapply(sample_names, 
                       function(id) mapping_file[,1] %in% id %>% which()) %>% as.vector() %>% mapping_file[.,]
...
# this tries to dereference the png symlink but doesn't work.
if (F){
  is.symlink <- function(paths) isTRUE(nzchar(Sys.readlink(paths), keepNA=TRUE))
  getwd()
  pngfile=paste0('rename_QC_meanexp_vs_freq',tmp_plateid,'.png')
  if (file.exists(pngfile)) {
    sprintf('1 pngfile exist %s', pngfile)
  } else {
    sprintf('1 pngfile exist %s', pngfile)
  }
  if (is.symlink(pngfile)) {
    pngfile <- Sys.readlink(pngfile)
  }
  if (file.exists(pngfile)) {
    sprintf('2 pngfile exist %s', pngfile)
  } else {
    sprintf('2 pngfile exist %s', pngfile)
  }
}

# pngfile=paste0('../QC_meanexp_vs_freq',tmp_plateid,'.png') #wu
