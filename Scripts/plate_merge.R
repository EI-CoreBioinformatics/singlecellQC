...
#'
#' @examples
plate_merge <- function(outdir, q_merge_col='est_counts', id_list, pipeline='snakemake'){
  # v1: id_list
  # > id_list
  ...
  # file_list <- id_list %>% strsplit(' ') %>% unlist %>% gsub('\\[','',.) %>% gsub('\\]','',.) %>% file.path
  if (pipeline=='snakemake') {
    file_list <- id_list %>% strsplit(" ")%>% unlist%>%gsub(',',"",.)
  }
  ...
  # write.table(final_matrix, paste0(outdir, q_merge_col, "all_plates.tsv"), sep = '\t')
  outfile = file.path(outdir, paste0(q_merge_col, "all_plates.tsv")) #wu
  write.table(final_matrix, outfile, sep = '\t') #wu
}
...
if(sys.nframe()==0){
  ...
  if(length(args)>=3){
    ...
    if (length(args)==3){
      ...
      plate_merge(args[1], args[2], args[3], args[4])
    }
  } else stop(">>> Missing args! <<<")
}
